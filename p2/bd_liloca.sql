...
CREATE DEFINER = root@localhost PROCEDURE inserir_produto (IN txtidcliente INT,IN txtbolo varchar(30),IN txtsalgado varchar(30),IN txtrefri varchar(30),IN txtquantidadeB INT(11),IN txtquantidadeS INT(11),IN txtquantidadeR INT(11),IN txtvalor INT(11) 	,IN txtvalorS INT(11) 	,IN txtvalorR INT(11)) 		BEGIN 
SET @txtidcliente = txtidcliente;
SET @txtbolo = txtbolo;
SET @txtsalgado = txtsalgado;
SET @txtrefri = txtrefri;
...
SET @txtvalorS = txtvalorS;
SET @txtvalorR = txtvalorR;
PREPARE STMT FROM "INSERT INTO produto (Cliente_idCliente, bolo_chocolate, salgado_coxinha, refrigeranteC, quantidade_boloC, quantidade_refriC, quantidade_salgadoC, valor_unit, valor_unitS, valor_unitR) VALUES (?,?,?,?,?,?,?,?,?,?)";
EXECUTE STMT USING @txtidcliente,@txtbolo,@txtsalgado,@txtrefri,@txtquantidadeB,@txtquantidadeS,@txtquantidadeR,@txtvalor,@txtvalorS,@txtvalorR;
END$$
-- Final Produto
...
CREATE DEFINER = root@localhost PROCEDURE editar_festa (IN txtidcliente INT, IN txtnome VARCHAR(30), IN txtidade INT(11), IN txtendereco VARCHAR(30), IN txttema VARCHAR(30), IN txtcores VARCHAR(30)) BEGIN
SET @txtidcliente = txtidcliente;
SET @txtnome = txtnome;
...
UPDATE festa SET nome = txtnome, idade = txtidade, endereco = txtendereco, tema = txttema, cores = txtcores WHERE Cliente_idCliente = txtidcliente;
END $$

CREATE DEFINER = root@localhost PROCEDURE inserir_festa (IN txtidcliente INT, IN txtnome VARCHAR(30), IN txtidade INT(11), IN txtendereco VARCHAR(30), IN txttema VARCHAR(30), IN txtcores VARCHAR(30)) BEGIN
SET @txtidcliente = txtidcliente;
SET @txtnome = txtnome;
SET @txtidade = txtidade;
SET @txtendereco = txtendereco;
...
PREPARE STMT FROM "INSERT INTO festa (Cliente_idCliente, nome, idade, endereco, tema, cores) VALUES (?,?,?,?,?,?)";
EXECUTE STMT USING @txtidcliente,@txtnome,@txtidade,@txtendereco,@txttema,@txtcores;
END $$
-- Final Festa
